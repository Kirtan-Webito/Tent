        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-border">
                <div>
                    ...
                    <p className="text-sm md:text-base text-muted-foreground mt-2 font-medium italic">Inventory management and deployment controls for this sector.</p>
                </div>
            ...
            <div className="bg-white rounded-[2.5rem] overflow-hidden border border-border shadow-sm">
                <div className="p-8 border-b border-border flex items-center justify-between flex-wrap gap-4">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-orange-100 border border-orange-200 flex items-center justify-center text-xl text-primary font-bold shadow-sm">
                            📦
                        </div>
                        <div>
                            <h3 className="text-xl font-bold text-foreground">Campsite Inventory</h3>
                            <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-0.5">Deployment Nodes: {sector.tents.length} Units</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 md:p-8">
                    {sector.tents.length === 0 ? (
                        <div className="py-24 text-center">
                            ...
                            <p className="text-muted-foreground font-medium italic">No active deployments detected. Use the Bulk Generator to initialize inventory.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
                            {sector.tents.map((tent: any) => (
                                <div
                                    key={tent.id}
                                    className="group relative p-4 rounded-2xl border border-border bg-card hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
                                >
                                    <DeleteTentButton tentId={tent.id} tentName={tent.name} />
                                    <div className="flex flex-col items-center text-center">
                                        <div className="text-[8px] font-black text-muted-foreground uppercase tracking-widest mb-1 group-hover:text-primary transition-colors">NODEID</div>
                                        <div className="font-black text-lg text-foreground group-hover:text-primary transition-colors tracking-tighter mb-1">{tent.name}</div>
                                        <div className="px-2 py-0.5 rounded-md bg-secondary text-[10px] font-bold text-muted-foreground uppercase">CAP: {tent.capacity}</div>
                                    </div>
                                </div>
                            ))}
                        </div>

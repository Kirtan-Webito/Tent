...
export default function TentsClient({ tents }: { tents: TentItem[] }) {
    const [selectedTent, setSelectedTent] = useState<TentItem | null>(null);
    const [isModalOpen, setIsModalOpen] = useState(false);

    const handleTentClick = (tent: TentItem) => {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4 animate-in slide-in-from-bottom-4 duration-500">
                {tents.length === 0 ? (
                    <div className="col-span-full py-20 glass rounded-[2rem] border-dashed border-white/10 text-center text-gray-500">
                        ...
                        No tents found. Go to Sectors to create some.
                    </div>
                ) : (
                    tents.map(item => (
                        <div
                            key={item.id}
                            onClick={() => handleTentClick(item)}
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-2">
                                <div className="flex justify-between items-center bg-black/20 p-2 rounded-lg border border-white/5">
                                    <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">SEC</span>
                                    <span className="text-[10px] text-white font-bold">{item.sector.name}</span>
                                </div>
